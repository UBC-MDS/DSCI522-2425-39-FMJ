...
summary_table = X_train.describe()
...
TODO: JASON TO DECIDE HOW TO VISUALIZE THE SUMMARY STATS TABLE
...
![Feature Distributions by Age Group (groups are not stacked)](../results/figures/INSERT_FIGURE_FILE_NAME){#fig-feat-distributions width=80%}

TODO: JASON TO UPDATE THE PATH TO THIS FIGURE
...
# TODO: FORGIVE TO ADD FILE NAME THEN DELETE THIS LINE
dc_results = pd.read_csv('../results/tables/INSERT_DUMMY_CLASSIFIER_CV_TABLE_FILE_NAME')
Markdown(dc_results.to_markdown())

dc_test_score = round(dc_results['test_score'].mean(), 5)
...
```{python}
#| label: tbl-cv-logreg
#| tbl-cap: Logistic regression cross validation scores

# TODO: FORGIVE TO ADD FILE NAME THEN DELETE THIS LINE
dc_results = pd.read_csv('../results/tables/INSERT_LOGREG_CV_TABLE_FILE_NAME')
Markdown(lr_results.to_markdown())

lr_mean_test_score = round(lr_results_df['test_score'].mean(), 5)
```
...
```{python}
#| label: tbl-cv-svc
#| tbl-cap: SVC cross validation scores

# TODO: FORGIVE TO ADD FILE NAME THEN DELETE THIS LINE
dc_results = pd.read_csv('../results/tables/INSERT_SVC_CV_TABLE_FILE_NAME')
Markdown(svc_results.to_markdown())
svc_mean_test_score = round(svc_results_df['test_score'].mean(), 5)
```

#### Comparing scores

Model mean CV scores:
Baseline: `{python} dc_test_score`
Logistic Regression: `{python} lr_mean_test_score`
SVC: `{python} scv_mean_test_score`
...
# svc_pipe = pickle.load(open(''../results/models/INSERT_SVC_MODEL_FILE_NAME'))

# svc_pipe.fit(X_train,y_train)
# test_score = round(svc_pipe.score(X_test,y_test), 3)
# test_score_rough = round(svc_pipe.score(X_test,y_test), 2) * 100
```
...
![Confusion matrix of the best model on test data](../results/figures/INSERT_FIGURE_FILE_NAME){#fig-confusion-matrix}

TODO: FORGIVE TO ADD PATH TO CONFUSION MATRIX
...
![ROC curve of the best model on test data](../results/figures/INSERT_FIGURE_FILE_NAME){#fig-roc}

TODO: FORGIVE TO ADD PATH TO ROC GRAPH
